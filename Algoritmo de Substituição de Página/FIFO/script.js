...
const filaPosProcessamento = document.getElementById("fila-paginas-processadas");
...
let conteudoRAM = []; // Páginas atualmente na RAM
...
async function simulacaoFIFO() {
    ...
    for (let step = 0; step < fila.length; step++) {
        ...
        const statusPagina = conteudoRAM.includes(conteudoPagina) ? "HIT" : "MISS";
        ...
        if (!conteudoRAM.includes(conteudoPagina)) { //Se a página não estiver na RAM
            // Page Fault: Página não está na RAM
            pageFaults++;
            if (conteudoRAM.length === quadrosRAM) {
                if (ultimoQuadroAtualizado > 2){
                    ultimoQuadroAtualizado = 0;
                }
                
                conteudoRAM[ultimoQuadroAtualizado] = conteudoPagina;
            } else {
                conteudoRAM.push(conteudoPagina);
            }
            
            // Atualizar o último quadro atualizado
            atualizarQuadro(ultimoQuadroAtualizado + 1, conteudoPagina);
            ultimoQuadroAtualizado++;
        } 

        // Atualizar a RAM na tabela
        atualizarTabelaPagina(step + 1, conteudoPagina, statusPagina, ultimoQuadroAtualizado);
...
function atualizarTabelaPagina(step, conteudoPagina, statusPagina, ultimoQuadroAtualizado) {
    ...
    for (let i = 0; i < quadrosRAM; i++) {
        ...
        paginaQuadro.textContent = conteudoRAM[i] !== undefined ? conteudoRAM[i] : "*";

        if (statusPagina === "HIT" && conteudoRAM[i] === conteudoPagina) {
            paginaQuadro.classList.add("page-hit");
        } else if (statusPagina === "MISS" && conteudoRAM[i] === conteudoPagina) {
...
async function processarRam(conteudoPagina){
    ...
    if (conteudoRAM.length === quadrosRAM) {
        descricao.innerText = "RAM está cheia. A página adicionada a mais tempo será será substituída pela página " + conteudoPagina + ".";
    } else {
...
document.getElementById("iniciar").addEventListener("click", simulacaoFIFO);
